...
class ContinuumRobotEnv(gym.Env):
    ...
    def step(self, action):
        ...
        done = self.done_fn(reward,self.num_step)
        # 检查是否完成
    ...
    def _get_observation(self):
        ...
        if self.control_mode==1:
          return np.array([
              end_pos[0],end_pos[1],end_pos[2],ball_pos[0], ball_pos[1], ball_pos[2]
          ], dtype=np.float64)
        elif self.control_mode==0:
    ...
    def default_reward_fn(self, distance):
        ...
        return reward
    def default_done_fn(self,reward,step):
        #done=reward>-0.5
        done=1
        return done
    def _calculate_reward(self):
        ...
        reward = self.reward_fn(distance)
        #reward = -distance + np.exp(-distance**2) * 5
